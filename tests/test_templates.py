...
class TemplatesTest(unittest.TestCase):
    ...
    def test_create_posts_editor_doc_with_sidecars_and_metadata(self):
        client, t = self._client([{"json": {"object": "template", "id": "tpl_1", "format": "editor"}}])
        editor_doc = {
            "type": "doc",
        ...
        self.assertEqual(call["method"], "POST")
        self.assertEqual(call["url"], "https://api.mailtea.app/v1/templates")
        # No "html" — the server renders it from editor_doc.
        self.assertEqual(
            json.loads(call["body"]),
            {
                "publication_id": "pub_1",
    ...
    def test_update_sends_nullable_library_fields_as_json_nulls(self):
        client, t = self._client([{"json": {"object": "template", "id": "tpl_1"}}])
        ...
        # The Nones are how a caller clears these, so they must reach the wire.
        self.assertEqual(
            json.loads(call["body"]),
            {
                "publication_id": "pub_1",
    ...
    def test_unpublish_posts_with_publication_id_and_no_body(self):
        client, t = self._client(
            [{"json": {"object": "template", "id": "tpl_1", "status": "draft"}}]
        )
        client.templates.unpublish("tpl_1", {"publication_id": "pub_1"})
        call = t.calls[0]
        self.assertEqual(call["method"], "POST")
        self.assertEqual(
            call["url"], "https://api.mailtea.app/v1/templates/tpl_1/unpublish?publication_id=pub_1"
        )
        self.assertIsNone(call["body"])
